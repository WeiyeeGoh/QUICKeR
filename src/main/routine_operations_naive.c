...
int main (int argc, char** argv) {
    ...
    while (time_expired == 0) {
        ...
        if (rand() % 100 < download_percentage){
            ...
            rv = download_and_decrypt (&reuse_session, database_keys[db_key_index], key_handle, &message, &message_length); 

            //exit(0);

            if (message_length <= 0) {
                printf("Message length is <= 0\n");
                exit(0);
                printf("Message Number: %d\n", total_messages);
                printf("DB Key Number: %d\n", db_key_index);
                rv = encrypt_and_upload(&reuse_session, key_handle, database_keys[db_key_index], reuse_message, reuse_message_size);
            }


            // Verify Message was correct
            // if (message_length != reuse_message_size && strncmp(message, reuse_message, message_length) != 0) {
            //     printf("Failed\n");
